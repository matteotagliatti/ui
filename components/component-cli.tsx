...
export function ComponentCLI({ name }: { name: string }) {
  ...
  const commands = {
    pnpm: `pnpm dlx shadcn@latest add ${site.url}/r/${name}.json`,
    npm: `npx shadcn@latest add ${site.url}/r/${name}.json`,
    yarn: `npx shadcn@latest add ${site.url}/r/${name}.json`,
    bun: `bunx --bun shadcn@latest add ${site.url}/r/${name}.json`,
  };
