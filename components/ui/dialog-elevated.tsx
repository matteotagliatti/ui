  ...
  return (
    <DialogContext.Provider
      value={{
        ...
        id: {
          title: `dialog-${useId()}`,
          description: `dialog-${useId()}`,
          content: `dialog-${useId()}`,
        },
      }}
    >
      {children}
...
const DialogRootContent: React.FC<DialogRootProps> = ({
  children,
  onOpenChange,
  ...props
}) => {
...
const DialogContent: React.FC<DialogContentProps> = ({
  ...
  return (
    <RadixDialogPrimitive.Content {...props}>
      {children}
...
type DialogStackContentProps = React.ComponentPropsWithoutRef<
  typeof Primitive.h2
> &
  React.HTMLAttributes<HTMLHeadingElement>;
const DialogStackContent: React.FC<DialogStackContentProps> = ({
  children,
  ...props
}) => {
...
type DialogStackTitleProps = React.ComponentPropsWithoutRef<
  typeof Primitive.h2
> &
  React.HTMLAttributes<HTMLHeadingElement>;
const DialogStackTitle: React.FC<DialogStackTitleProps> = ({
  children,
  ...props
}) => {
  const { id } = useDialogContext();
...
type DialogStackDescriptionProps = React.ComponentPropsWithoutRef<
  typeof Primitive.h2
> &
  React.HTMLAttributes<HTMLHeadingElement>;
const DialogStackDescription: React.FC<DialogStackDescriptionProps> = ({
  children,
  ...props
}) => {
  const { id } = useDialogContext();
...
const DialogStackRemove: React.FC<DialogStackRemoveProps> = ({
  dialogId,
  children,
  ...props
}) => {
...
const DialogStackAdd: React.FC<DialogStackAddProps> = ({
  dialogId,
  children,
  ...props
}) => {
...
export type {
  ...
  DialogStackRemoveProps,
  DialogStackAddProps,
  DialogContentProps,
  DialogCloseProps,
  DialogTitleProps,
  DialogDescriptionProps,
  DialogSharedItemProps,
};
